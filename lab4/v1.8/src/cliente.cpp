...
string Cliente::toString() {
    ...
    + "Calle: " + this->calle + "\n" + "Número de puerta: " + this->nroPuerta + "\n" 
    + "Ciudad: " + this->ciudad + "\n";
    return res; 
}
...
DataCliente Cliente::getDataCliente(){
    return DataCliente(this->nickname, this->contrasena, this->fechaNacimiento, this->calle, this->nroPuerta, this->ciudad);
}

vector<DataCompra> Cliente::getCompraExpediente(){
    vector<DataCompra> expediente;
    map<string, Compra*>::iterator it;
    for (it = this->compras.begin(); it != this->compras.end(); ++it){
        expediente.push_back(DataCompra(it->second->getMonto(), it->second->getFecha(), it->second->getNicknameProductos()));
    }
    return expediente;
}