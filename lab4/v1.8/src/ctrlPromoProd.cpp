...
void CtrlPromoProd::realizarCompra(string nicknameCliente, vector<DataPromoProducto> productosCompra, DataFecha fecha, float monto) {
    ...
    for (unsigned int i = 0; i < productosCompra.size(); i++) {
        ...
        if(this->enviosPendientes.count(prod->getVendedor()->getNickname()) == 0){
            set<CompraProducto*> pendientes;
            pendientes.insert(cProd);
            this->enviosPendientes[prod->getVendedor()->getNickname()] = (pendientes);
        } else {
...
//Enviar Producto
bool CtrlPromoProd::noHayEnviosPendientes(){
    return (this->enviosPendientes.empty());
}
...
set<string> CtrlPromoProd::getProductosConEnviosPendientesDeVendedor(string vendedor){
    set<string> productos;
    set<CompraProducto*>::iterator it;
    for(it = this->enviosPendientes[vendedor].begin(); it != this->enviosPendientes[vendedor].end(); ++it){
        productos.insert((*it)->getProducto()->getCodigo());
    }
    return (productos);
}
...
vector<DataEnvioPendiente> CtrlPromoProd::getSolicitudesDeEnviosParaProducto(string vendedor,string codigo){
    ...
    set<CompraProducto*>::iterator it;
    for(it = this->enviosPendientes[vendedor].begin(); it != this->enviosPendientes[vendedor].end(); ++it){
        if((*it)->getProducto()->getCodigo().compare(codigo) == 0){
            solicitudes.push_back(DataEnvioPendiente((*it)->getCompra()->getCodigo(), (*it)->getCompra()->getNickCliente(), (*it)->getCompra()->getFecha()));
        }
    }
    return solicitudes;
}

void CtrlPromoProd::marcarComoEnviado(string vendedor, string codigo, string solicitado){
    set<CompraProducto*>::iterator it = this->enviosPendientes[vendedor].begin();
    while ((*it)->getProducto()->getCodigo().compare(codigo) != 0 || (*it)->getCompra()->getCodigo().compare(solicitado) != 0){
        ++it;
    }
    (*it)->setEnviado();
    this->enviosPendientes[vendedor].erase(it);
    if (this->enviosPendientes[vendedor].empty()){
        this->enviosPendientes.erase(vendedor);
    }
}
