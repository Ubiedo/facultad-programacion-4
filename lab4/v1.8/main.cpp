...
void main_expedienteDeUsuario(IUsuarios*);
...
int main() {
    ...
    while (continuar) {
        ...
        else if (entrada.compare("expedienteDeUsuario") == 0)
            main_expedienteDeUsuario(iUsrs);
...
void main_comandos() {
    ...
    << "realizarCompra: Realiza una compra\n"
    << "enviarProducto: Marca Producto como enviado\n"
    << "expedienteDeUsuario: Muestra el expediente del usuario\n"
        ...
        if (nicknameClientes.size() != 0) {
            ...
            while (continuar) {
                ...
                while (productoInvalido) {
                    ...
                    getline(cin, aux);
                    unsigned int i = 0;
                    while ((i > 0) && (i < productosDisponibles.size()) && (codigoProducto.compare(productosDisponibles[i].getCodigo()) != 0))
                        i++;
                    if (i < productosDisponibles.size())
...
void main_enviarProducto(IPromoProd* iPP){
    if(iPP->noHayEnviosPendientes()){
        ...
        set<string> productos = iPP->getProductosConEnviosPendientesDeVendedor(nickname);
        ...
        while (procesoInvalido) {
            ...
            getline(cin, aux);
            it = productos.begin();
            while ((it != productos.end()) && (codigo.compare(*it) != 0)){
                ++it;
            }
            if (it != productos.end()){
                procesoInvalido = false;
        ...
        vector<DataEnvioPendiente> solicitudes = iPP->getSolicitudesDeEnviosParaProducto(nickname, codigo); // si no se puede string[2] usar un nuevo dataType, dataEnvio, string nickname, dataFecha fecha
        cout << "Solicitudes de envios pendientes del producto " << codigo <<":" << endl;
        unsigned int i;
        for(i = 0; i < solicitudes.size(); i++){
            cout << i+1 << ") Solicitado por " << solicitudes[i].getNombreCliente() << ", el dia " << solicitudes[i].getFecha().toString() << endl;
        }
        int solicitud;
        procesoInvalido = true;
        while (procesoInvalido) {
            cout << endl << "Seleccionar numero de solicitud: ";
            scanf("%d", &solicitud);
            getline(cin, aux);
            if (solicitud >= 1 && solicitud <= solicitudes.size()){
                procesoInvalido = false;
            } else {
        ...
        iPP->marcarComoEnviado(nickname, codigo, solicitudes[solicitud].getSolicitud());
        cout << endl << "¡Producto enviado con exito!\n";
    }
}

void main_expedienteDeUsuario(IUsuarios* iUsrs){
    /************************LISTADO DE USUARIOS***********************/
    vector<string> nicknameUsuarios = iUsrs->getUsuarios();
    ...
    if (i == 0){
        ...
        bool nicknameInvalido = true;
        while(nicknameInvalido){
            cout << endl << "Ingrese el usuario a consultar: ";
            cin >> nickname;
            getline(cin,aux);
            unsigned int i = 0;
            while((i < nicknameUsuarios.size()) && (nickname.compare(nicknameUsuarios[i]) != 0))
            ...
            if(i < nicknameUsuarios.size())
                ...
                cout << endl << "[ERROR] No existe el usuario " << nickname << "\n";
        }
        /***********************LISTADO DE INFO USUARIO********************/
        cout << endl;
        bool boolCliente = iUsrs->esCliente(nickname);
        // SI ES CLIENTE
        if(boolCliente){
            DataCliente infoBasica = iUsrs->getDataCliente(nickname);
            vector<DataCompra> infoCompras = iUsrs->getCompraExpediente(nickname);
            cout << "Información del Cliente " << infoBasica.getNickname() << ":"<< endl;
            cout << "Fecha de nacimiento: " << infoBasica.getFechaNacimiento().toString() << endl;
            cout << "Ciudad: " << infoBasica.getCiudad() << endl;
            cout << "Calle: " << infoBasica.getCalle() << endl;
            cout << "Número de puerta: " << infoBasica.getNroPuerta() << endl;
            cout << "Compras realizadas: " << endl;
            for (int i = 0; i < infoCompras.size(); i++){
                cout << "Compra " << i+1 << ":" << endl; 
                cout << "Fecha: " << infoCompras[i].getFecha().toString() << endl;
                cout << "Importe: " << infoCompras[i].getMonto() << endl;
                cout << "Productos comprados: " << endl;
                vector<string> productos = infoCompras[i].getInfoProductos();
                for (int j = 0; j < productos.size(); j++){
                    cout << "Producto " << j+1 << ": " << productos[j] << endl;
                }
            }
            if(infoCompras.size() == 0){
                cout << "No hay compras realizadas por el cliente " << endl;
            }
        // SI NO ES CLIENTE => ES VENDEDOR
        } else {
            DataVendedor infoBasica = iUsrs->getDataVendedor(nickname);
            cout << "Información del Vendedor " << infoBasica.getNickname() << ":" << endl;
            cout << "Fecha de nacimiento: " << infoBasica.getFechaNacimiento().toString() << endl;
            cout << "Número de rut: " << infoBasica.getNroRut() << endl;
            
            set<string>::iterator it;
            set<string> productos = iUsrs->getNombresDeProductosEnVentaDeVendedor(nickname);
            cout << "Productos en venta del Vendedor: " << endl;
            for (it = productos.begin(); it != productos.end(); ++it){
                cout << *it << endl;
            }
            set<string> promociones = iUsrs->getNombresDePromocionesVigenteDeVendedor(nickname);
            cout << "Promociones vigentes del Vendedor: " << endl;
